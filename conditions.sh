...
if [[ ( $((NUMBER3 % 2)) -eq 1 && "$NUMBER3" -gt 1 ) || "$NUMBER3" -eq 2 || "$NUMBER3" -ne 0 ]]; then
    echo "${NUMBER3} is prime number"
elif [ $((NUMBER3 % 2)) -eq 0 ] && [ "$NUMBER3" -ne 2 ] && [ "$NUMBER3" -gt 1 ] && [ "$NUMBER" -ne 0 ]; then
    echo "$NUMBER3 is composite Number"
else
    echo "$NUMBER3 is neither prime or composite"
fi